...
def get_community_posts(max_results=4):
    """Fetch latest channel activity including community posts via activities API."""
    posts = []
    # Try activities endpoint for community posts
    ...
    for item in data.get('items', []):
        ...
        if activity_type in ('bulletinPost', 'recommendation', 'channelItem', 'social'):
            ...
            posts.append({
                'text': text[:300] if text else 'Check out our latest update on YouTube!',
                'image': image,
                'published': pub,
...
def video_card(v, is_short=False):
    ...
    return f"""
    <div class="video-card" onclick="openModal('{v['id']}','{title_safe}')">
...
def post_card(p):
    img_html = f'<img src="{p["image"]}" alt="Anime1Point Community Post" loading="lazy" style="width:100%;border-radius:6px;margin-top:10px;"/>' if p.get('image') else ''
    return f"""
    <a class="post-card" href="{p['url']}" target="_blank" rel="noopener">
      <div class="post-date">{p['published']}</div>
      <p class="post-text">{p['text'][:280]}{'...' if len(p['text']) >= 280 else ''}</p>
      {img_html}
    </a>"""

def build_html(videos, shorts, posts):
    ...
    if posts:
        ...
        post_cards_html = '<p style="color:#9ca3af;padding:20px 0;">Community posts are updated automatically when new posts are available. <a href="https://www.youtube.com/@anime1point/community" style="color:#60a5fa" target="_blank" rel="noopener">View all community posts on YouTube &rarr;</a></p>'
...
<div style="text-align:center;margin-bottom:40px">
<a href="https://www.youtube.com/@anime1point/community" target="_blank" rel="noopener" class="cta-btn" style="background:#7c3aed">&#9654; See All Posts on YouTube</a>
</div>
...
<p style="margin-top:10px"><a href="https://www.youtube.com/@anime1point" target="_blank">YouTube</a> &bull; <a href="https://www.youtube.com/@anime1point/videos" target="_blank">Videos</a> &bull; <a href="https://www.youtube.com/@anime1point/shorts" target="_blank">Shorts</a> &bull; <a href="https://www.youtube.com/@anime1point/community" target="_blank">Community</a> &bull; <a href="https://www.youtube.com/@anime1point/playlists" target="_blank">Playlists</a></p>
...
if __name__ == '__main__':
    ...
    html = build_html(videos, shorts, posts)
